  ...
  Store: {
    ...
    Prompt: {
      ...
      Topic:
        "你是一个对话标题生成器。\n请根据以下对话内容生成简洁标题。\n要求：\n- 4到8个中文字符\n- 只输出标题本身\n- 不要标点\n- 不要解释\n- 不要语气词\n- 不要加引号\n用户对话内容：\n{{user_messages}}\n用户最近确认的助手回复（可为空）：\n{{assistant_message}}",
      Summarize:
        "你是对话上下文压缩器。根据下面的「已有摘要」和「新对话」输出一份更新后的摘要，供后续对话使用。\n\n## 要求\n- 提取：用户核心需求、目标、偏好、已确认的事实或结论\n- 保留：关键上下文（项目名、技术栈、路径、关键数字等）\n- 忽略：闲聊、重复、无关内容\n- 若有「已有摘要」，请与新内容合并并去重，不要简单罗列\n- 用简洁自然语言、分点列出，总长 200 字以内\n\n## 已有语义状态（可为空）\n{{previous_summary}}\n\n## 新的用户发言\n{{user_messages}}\n\n## 用户已确认的助手结论（可为空）\n{{assistant_messages}}\n\n请直接输出更新后的摘要，不要解释或加前缀。",
    },
  },
  Copy: {
    Success: "已写入剪贴板",
